...
the runner's own resume machinery into `<worktree>/.alder-ext-runner/`, and
starts a detached tmux session running the tier's engine with the prompt
file's contents (verbatim) as the engine's final argument. It prints the
...
rung on the other ladder, and says so on stderr. `budget` reads
trailing-window token spend per provider off the transcripts the CLIs
already write (`CODEX_HOME`, `CLAUDE_CONFIG_DIR` move where it looks).
...
final argument and the tier is still what the session records.
...
- config file: `$ALDER_EXT_RUNNER_CONFIG`, or
  ...
  table.
- state (the rate-limit file): `$ALDER_EXT_RUNNER_STATE_DIR`, or
...
- it stamps only its own names into sessions (`ALDER_EXT_RUNNER_HANDLE`,
  ...
  (`.alder-ext-runner/`) into a worktree;
